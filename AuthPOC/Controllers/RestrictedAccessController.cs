...
public class RestrictedAccessController : ControllerBase
{
    ...
    public IActionResult GetTokenExpiry()
    {
        ...
        {
            DateTime? result = _auth.GetTokenExpiry(this.HttpContext.Request.Headers["Authorization"].ToString());
            return Ok($"Current token expires at: {result}");
        }
    }
